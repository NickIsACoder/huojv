			...
			<div class="data-content-panel">
                ...
                <div v-if="orgType<6">
                    <div class="data-title-box data-title-line" style="margin-top: 0;">
                        <span class="title">工程节点按期完成率</span>
                    </div>
                    <div class="huojv-costContract-chart-panel max" ref="chart1"></div>
                </div>
				<!-- 拿地-修详规批复 -->
				<div class="data-title-box data-title-line" style="margin-top: 0;">
					<span class="title">工程节点逾期情况</span>
				</div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getChartsData() {
                request({
                    url: `/app-api/engineer/getNodeCompletionRate`,
                    method: 'post',
            ...
            getTableData1(){
                request({
                    url: `/app-api/engineer/getNodeOverview`,
                    method: 'post',
                    data:{orgId:this.orgId,current: 1, size: 9}
                }).then(({ data }) => {
                    if (data.code === 0 && data.data) {
                        let list = data.data;
            ...
            // 点击查看更多
            goMore() {
                this.$router.push({path: '/report/engineer/more',
                    query: {
                        orgId: this.orgId,
