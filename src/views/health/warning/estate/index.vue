          ...
          <div class="list-data1">
            <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" >
              <el-table stripe
                ...
                <el-table-column prop="arrears" :label="actTab==0?'金额(万)':'金额(万)'" align="center" width="70">
                  <template slot-scope="scope">
                    <span>{{scope.row.arrears|changeVal('amount')}}</span>
                    <!-- <label v-if="actTab==0" v-text="(scope.row.arrears.amountTenThousand || '-')"></label>
                    <label v-else v-text="(scope.row.arrears.amountBillion || '-')"></label> -->
                  </template>
                </el-table-column>
...
import { regexNum } from '@/utils/common'
import $ from 'jquery';
...
export default {
  ...
  methods: {
    ...
    // 获取计划进度数据
    getList(isRefresh) {
      let url = '/app-api/healthlife/property/getWarning';
      // let params = {
      //   size: this.size,
      //   current: this.current,
      //   orgId: this.orgId,
      //   queryPeriod: '',
      //   type: this.actTab+1 //类型(1:管理费;2:水电费)
      // };
      ...
      request({
        ...
        data:{
          ...
          current: this.current,
          orgId: this.orgId,
          queryPeriod: ['年度','季度','月度'][this.timeTab],
          orderType: 'DESC',
          type: this.actTab //类型0:管理费;1:水电费
        },
      })
        .then(({ data }) => {
